...
export async function syncUserProfiles(
	...
	const tasks = items.map(graphUser =>
		limit(async () => {
			const dbItem = db.find(item => item.koksmat_masterdata_id === graphUser.userPrincipalName);
			if (dbItem) {
				if (options.createOnly) return;
				// --------------------------------------------------------------------------------
				// Updating
				// --------------------------------------------------------------------------------
				app.log.highlight(`User : ${graphUser.userPrincipalName}`, dbItem.id, 'Updating item');
				try {
					...
					const updatedRecord = await prisma.userProfile.update({
						...
						data: {
							...
							updated_by: 'system',
							updated_at: new Date(),
							CurrentEmail: graphUser.onPremisesExtensionAttributes?.extensionAttribute6,
							OldEmail: graphUser.onPremisesExtensionAttributes?.extensionAttribute9,
							OnPremisesId: graphUser.onPremisesExtensionAttributes?.extensionAttribute11,
							email: graphUser.userPrincipalName,
							company: graphUser.Companyname,
							companykey: graphUser.onPremisesExtensionAttributes?.extensionAttribute4,
							country: graphUser.Country ? graphUser.Country : 'Unknown',
							isExternal:
								graphUser.onPremisesExtensionAttributes?.extensionAttribute4?.endsWith('E') ??
								false,
						},
					});
					...
					await app.writeSyncLogError('update', { sharePointItem: graphUser }, error as Error);
				}
			} else {
				// --------------------------------------------------------------------------------
				// Creating
				...
				try {
					...
					app.log.error('Syncronising User ', graphUser.userPrincipalName, ' see log for details');
					await app.writeSyncLogError('create', { sharePointItem: graphUser }, error as Error);
				}
			}
		}),
	);
